  ...
  return (
    <aside style={{width:236,padding:"18px 14px",background:C.surface,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",gap:4,overflowY:"auto",flexShrink:0,boxShadow:"2px 0 10px rgba(0,0,0,0.02)"}}>
      ...
      {NAV.map(({iconName,label,path},i)=>{
        ...
        return (
          <button key={label} onClick={()=>navigate(path)}
            style={{display:"flex",alignItems:"center",gap:10,padding:"11px 13px",borderRadius:12,border:"none",background:isAct?"rgba(var(--primary-rgb),0.08)":"transparent",borderLeft:`2px solid ${isAct?C.teal:"transparent"}`,color:isAct?C.teal:C.muted,cursor:"pointer",fontSize:14,fontWeight:isAct?600:500,transition:"all 0.12s",fontFamily:"'Inter',sans-serif",textAlign:"left",width:"100%",animation:`slideIn 0.4s ease ${0.04*i}s both`}}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        <button onClick={()=>navigate("/contact-us")} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 13px",borderRadius:12,border:`1px solid ${C.border}`,background:C.surface,color:C.text,cursor:"pointer",fontSize:14,fontWeight:600,transition:"all 0.12s",fontFamily:"'Inter',sans-serif",textAlign:"left",width:"100%"}}
        ...
        <div
          role="button"
          tabIndex={0}
          style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 13px",borderRadius:12,border:`1px solid ${C.border}`,background:C.surface,width:"100%",transition:"all 0.12s",cursor:"pointer"}}
          onMouseEnter={e=>e.currentTarget.style.background=C.surfaceHover}
          onMouseLeave={e=>e.currentTarget.style.background=C.surface}
        ...
        >
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <span style={{fontSize:14,fontWeight:600,color:C.text,fontFamily:"'Inter',sans-serif"}}>{theme === 'dark' ? 'Dark Mode' : 'Light Mode'}</span>
          </div>
        ...
        <button onClick={logout} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 13px",borderRadius:12,border:`1px solid ${C.border}`,background:C.surface,color:C.red,cursor:"pointer",fontSize:14,fontWeight:600,transition:"all 0.12s",fontFamily:"'Inter',sans-serif",textAlign:"left",width:"100%"}}
